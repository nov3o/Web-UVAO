...
//all scenes
	...
	choices[1] = [
		...
		[,,,'Нет, не детский фасон (вроде как шапочка явно на взрослую, но я не мог представить себе взрослую девушку с таким помпоном на голове), а две прорези по обе стороны от него. Тут было два варианта: либо кому-то наставили рога, либо обладательница шапочки была настоящая нека.'],
		...
		[,,'street.jpg','Я вспомнил, что та девушка побежала в сторону парка, потому отправился в его сторону. Что ж, погуляю с полчасика, все равно заняться мне особо нечем. Не найду - пойду домой. Вдали в свете фонарей мне привиделась человеческая фигурка, явно принадлежавшая молодой девушке.'],
	...
	choices[1010010] = [
		[,,'koczka_smile.jpg','- Не знаю, я тут по парку гулял, искал маленьких неко-девочек.','dialogue'],
		...
		[,,,'Девочка рассмеялась и скрылась из виду. Я же остался стоять на краю зеркального отражения. Может она права? Я прикрыл глаза на мгновение… И тут же получил сильный толчок в спину, окунувшись с головой в воду. '],
		[,,'black','Я открыл глаза и резко поднялся.'],
		[,,'slavia_close.jpg','Девушка, стоявшая подле, от моего движения, отпрыгнула, испуганно закрывшись подносом. Я огляделся по сторонам: пустой Макдональдс, вечер, около меня стоит кассирша, та самая, с длинными косами.'],
...
//functions
	...
	function dialogue(list) {
		...
		document.getElementById('text').innerHTML = list[3]
	}
