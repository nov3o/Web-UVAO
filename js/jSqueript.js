$(document).ready(function () {
	...
	fill = ''
	backImg = ''
	music = '../../static/music/theme2.mp3'

	//all scenes
		scenes = [
			['../../static/music/theme2.mp3','black.jpg','<i>За окном медленно падает снег. В круговороте кружатся снежинки, которые еще не знают, что через мгновение разлетятся в разные стороны и уже никогда не будут вместе, как прежде.</i>','dialogue'],
	...
	//just functions
		function next() {
			...
			if (list[1] != backImg) {
				backImg = list[1]
				$('.full-screen').css('background-image', 'url(../../static/images/' + backImg +')')
			}
			...
			switch(mode) {
				...
				case 'badEnd':
					...
					break
				case 'empty':
					$('.message').eq(0).css('display', 'none')
					$('.back').eq(0).css('opacity', '0')
					$('.full-page-text').eq(0).css('display', 'none')
					break
				case 'slow-down':
					slowDown(list)
					break
		...
		function dialogue(list) {
			if (list[3] == 'dialogue') {
				$('.message').eq(0).css('display', 'block')
				$('.back').eq(0).css('opacity', '0')
				$('.full-page-text').eq(0).css('display', 'none')
			}
			$('#text').html(list[2])
		...
		function fullPageText(list) {
			page = $('.full-page-text').eq(0)
			if (list[3] == 'full-page-text') {
				$(page).html('')
				$('.message').eq(0).css('display', 'none')
				$('.back').eq(0).css('opacity', '0.7')
				$('.full-page-text').eq(0).css('display', 'block')
			}
			p = $('<p></p>').text(list[2])
			$(page).append(p)
		}
		...
		function choice(ch) {
			...
			$('.back').eq(0).css('opacity', '0')
			$('.full-page-text').eq(0).css('display', 'none')
			$(choiceListClone).replaceWith(choiceList)
			$(choiceList).append(emptyButton)
			yourChoices += ch
			switch (yourChoices) {
				case '1000':
					yourChoices = '10'
		...
		function choicePage(list) {
			...
			for (i=0; i<vars.length; i++) {
				...
				button.setAttribute("onClick", "choice("+i+")");
		...
		function badEnd(list) {
			words = list[2]
			endNode = $('.bad-end').eq(0)
			$('.message').css('display', 'none')
			$('.back').css('opacity', '0.7')
			$(endNode).css('display', 'block')
			endChilds = $(endNode).children()
			summaryHeight = 0
			for (i=0; i<words.length; i++) {
				if (words[i] != undefined) {
					$(endChilds).eq(i*2+1).html(words[i])
					summaryHeight += $(endChilds).eq(i*2+1).outerHeight()
				}
			}
			$(endNode).css('top', (innerHeight-summaryHeight)/3 + 'px')
		}
		...
		function slowDown (list) {
			words = list[2]
			$('.slow-down').eq(0).css('display','block')
			$('.message').eq(0).css('display','none')
			slowDownNode = $('.slow-down').eq(0)
			slowDownNode.style.animation = 'slowdn 5s ease-out 0s 1 normal'
			$('.main-text').eq(0).html(words[0])
			$('.add-text').eq(0).html(words[1])
		...
		} // I can to refactore this a little more
